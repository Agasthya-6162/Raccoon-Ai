...
stable-linux.yml in the separate `raccoon-builder/` repo,
which runs a GitHub Action that builds the AppImage you see on our website.


# raccoon AppImage Creation Script

This script automates the process of creating an AppImage for the  raccoon Editor using Docker. It works on macOS and Linux platforms.
## Requirements
...
5.  **Result:**
    ...
    After the script completes, it will generate an AppImage named `raccoon-x86_64.AppImage` (or similar, depending on your architecture) in the current directory.
...
*   **AppImage Creation:**
    *   Creates the `raccoonApp.AppDir` structure.
    *   Copies binaries, resources, and the `.desktop` entry.
    *   Copies `raccoon.desktop` and `raccoon.png`.
