...
# Check if raccoon binary exists in current directory
if [ ! -f "./raccoon" ]; then
    echo "Error: raccoon binary not found in current directory"
    exit 1
fi

# Check if icon exists
if [ ! -f "./raccoon.png" ]; then
    echo "Error: raccoon.png icon not found in current directory"
    exit 1
fi
...
cp -v ./raccoon "$APP_DIR/usr/bin/"

# Copy the icon to required locations
cp -v ./raccoon.png "$APP_DIR/raccoon.png"
cp -v ./raccoon.png "$APP_DIR/usr/share/icons/hicolor/256x256/apps/raccoon.png"

# Copy dependencies with error checking
echo "Copying dependencies..."
for lib in $(ldd ./raccoon | grep "=> /" | awk '{print $3}'); do
    if [ -f "$lib" ]; then
...
Type=Application
Categories=Utility;
Comment=raccoon Linux Application
EOF
then
