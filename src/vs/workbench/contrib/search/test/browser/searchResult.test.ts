...
suite('SearchResult', () => {
	...
	test('replace should remove the file match', function () {
		const raccoonPromise = Promise.resolve(null);
		instantiationService.stub(IReplaceService, 'replace', raccoonPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange))]);
		...
		return raccoonPromise.then(() => assert.ok(testObject.isEmpty()));
	});
	...
	test('replaceAll should remove all file matches', function () {
		const raccoonPromise = Promise.resolve(null);
		instantiationService.stubPromise(IReplaceService, 'replace', raccoonPromise);
		const testObject = aSearchResult();
		addToSearchResult(testObject, [
			aRawMatch('/1',
				new TextSearchMatch('preview 1', lineOneRange)),
