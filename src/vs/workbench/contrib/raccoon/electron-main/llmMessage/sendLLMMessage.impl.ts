...
import { ChatMode, displayInfoOfProviderName, ModelSelectionOptions, OverridesOfModel, SettingsOfProvider } from '../../common/raccoonSettingsTypes.js';
...
const newOpenAICompatibleSDK = async ({ settingsOfProvider, providerName, includeInPayload }: { settingsOfProvider: SettingsOfProvider, providerName: ProviderName, includeInPayload?: { [s: string]: any } }) => {
	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				'HTTP-Referer': 'https://raccoon.ai', // Optional, for including your app on openrouter.ai rankings.
				'X-Title': 'Raccoon AI', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
	...
	else throw new Error(`raccoon providerName was invalid: ${providerName}.`)
}
	...
	openai.chat.completions
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Raccoon AI: Response from model was empty.', fullError: null })
			}
			else {
		...
		.catch(error => {
			if (error instanceof OpenAI.APIError && error.status === 401) { onError({ message: invalidApiKeyMessage(providerName), fullError: error }); }
			else if (error instanceof OpenAI.APIError && error.status === 429 || (error?.message && error.message.includes('429'))) { onError({ message: 'Rate limit reached. You exceeded your current quota or sent too many requests.', fullError: error }); }
			else { 
				let safeMsg = error instanceof Error ? error.message : error + '';
				if (safeMsg.length > 200) safeMsg = safeMsg.substring(0, 200) + '...';
				onError({ message: safeMsg, fullError: error }); 
			}
		})
}
...
const sendAnthropicChat = async ({ messages, providerName, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, overridesOfModel, modelName: modelName_, _setAborter, separateSystemMessage, chatMode, mcpTools }: SendChatParams_Internal) => {
	...
	stream.on('error', (error) => {
		...
		else if (error instanceof Anthropic.APIError && error.status === 429 || (error?.message && error.message.includes('429'))) { onError({ message: 'Rate limit reached. You exceeded your current quota or sent too many requests.', fullError: error }) }
		else { 
			let safeMsg = error instanceof Error ? error.message : error + '';
			if (safeMsg.length > 200) safeMsg = safeMsg.substring(0, 200) + '...';
			onError({ message: safeMsg, fullError: error }) 
...
const sendGeminiChat = async ({
	...
	// instance
	const genAI = new GoogleGenAI({ apiKey: thisConfig.apiKey, apiVersion: 'v1beta' });
	...
	genAI.models.generateContentStream({
		...
		.then(async (stream) => {
			...
			for await (const chunk of stream) {
				...
				// message (chunk.text contains all non-thought text parts in this chunk)
		...
		.catch(error => {
			...
			if (typeof message === 'string') {
				...
				if (error.message?.includes('API key')) {
					...
					onError({ message: 'Rate limit reached. You exceeded your current quota or sent too many requests.', fullError: error });
				}
				else {
				...
				onError({ message: 'An unknown error occurred.', fullError: error });
			}
		})
};
