...
export class RaccoonMainUpdateService extends Disposable implements IRaccoonUpdateService {
	...
	async check(explicit: boolean): Promise<raccoonCheckUpdateRespose> {
		...
		if (this._updateService.state.type === StateType.Ready) {
			// Update is ready
			return { message: 'Restart Raccoon to update!', action: 'restart' } as const
		}
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<raccoonCheckUpdateRespose> {
		try {
			const response = await fetch('https://api.github.com/repos/raccooneditor/binaries/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Raccoon is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Raccoon is up-to-date!'
					}
				}
				else {
