...
import { ChatMode } from '../raccoonSettingsTypes.js';
...
export const raccoonPrefixAndSuffix = ({ fullFileStr, startLine, endLine }: { fullFileStr: string, startLine: number, endLine: number }) => {
...
// export const aiRegex_computeReplacementsForFile_userMessage = async ({ searchClause, replaceClause, fileURI, raccoonFileService }: { searchClause: string, replaceClause: string, fileURI: URI, raccoonFileService: IRaccoonFileService }) => {

// 	// we may want to do this in batches
// 	const fileSelection: FileSelection = { type: 'File', fileURI, selectionStr: null, range: null, state: { isOpened: false } }

// 	const file = await stringifyFileSelections([fileSelection], raccoonFileService)
