...
export class MetricsService implements IMetricsService {
	...
	constructor(
		...
		this.metricsService = ProxyChannel.toService<IMetricsService>(mainProcessService.getChannel('raccoon-channel-metrics'));
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'raccoonDebugInfo',
			f1: true,
			title: localize2('raccoonMetricsDebug', 'Raccoon AI: Log Debug Info'),
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		notifService.info(`Raccoon AI Debug info:\n${JSON.stringify(debugProperties, null, 2)}`)
	}
})
