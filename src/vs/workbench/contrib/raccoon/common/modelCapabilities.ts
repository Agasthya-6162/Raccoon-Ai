...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, SendableReasoningInfo } from './raccoonSettingsTypes.js';
import { ProviderName } from './raccoonConstants.js';
...
type raccoonStaticProviderInfo = { // doesn't change (not stateful)
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies raccoonStaticModelInfo
...
} as const satisfies { [s: string]: Partial<raccoonStaticModelInfo> }




// keep modelName, but use the fallback's defaults
const extensiveModelOptionsFallback: raccoonStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {

	const lower = modelName.toLowerCase()

	const toFallback = <T extends { [s: string]: Omit<raccoonStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: raccoonStaticModelInfo & { modelName: string, recognizedModelName: string } => {
	...
	if (lower.includes('gemini') && (lower.includes('3.1'))) return toFallback(geminiModelOptions, 'gemini-3.1-pro-preview')
	if (lower.includes('gemini') && (lower.includes('3'))) return toFallback(geminiModelOptions, 'gemini-3-flash-preview')
	if (lower.includes('gemini') && (lower.includes('2.5'))) return toFallback(geminiModelOptions, 'gemini-2.5-pro')
	if (lower.includes('gemini')) return toFallback(geminiModelOptions, 'gemini-2.5-pro')
...
const anthropicModelOptions = {
	'claude-opus-4-5-20251120': { // Latest Claude Opus 4.5
		...
		reasoningCapabilities: {
			...
			reasoningSlider: { type: 'budget_slider', min: 1024, max: 128_000, default: 8192 },
		},
	},
	'claude-sonnet-4-5-20251120': { // Latest Claude Sonnet 4.5
		contextWindow: 200_000,
		reservedOutputTokenSpace: 8_192,
...
} as const satisfies { [s: string]: raccoonStaticModelInfo }

const anthropicSettings: raccoonStaticProviderInfo = {
...
const openAISettings: raccoonStaticProviderInfo = {
...
const xAISettings: raccoonStaticProviderInfo = {
...
const geminiSettings: raccoonStaticProviderInfo = {
	modelOptions: geminiModelOptions,
	modelOptionsFallback: (modelName) => {
		const lower = modelName.toLowerCase()
		if (lower.includes('3.1') && lower.includes('pro')) return { modelName: 'gemini-3.1-pro-preview', recognizedModelName: 'gemini-3.1-pro-preview', ...geminiModelOptions['gemini-3.1-pro-preview'] }
		if (lower.includes('3.1') && lower.includes('flash') && lower.includes('lite')) return { modelName: 'gemini-3.1-flash-lite-preview', recognizedModelName: 'gemini-3.1-flash-lite-preview', ...geminiModelOptions['gemini-3.1-flash-lite-preview'] }
		if (lower.includes('3') && lower.includes('flash')) return { modelName: 'gemini-3-flash-preview', recognizedModelName: 'gemini-3-flash-preview', ...geminiModelOptions['gemini-3-flash-preview'] }
		if (lower.includes('2.5') && lower.includes('pro')) return { modelName: 'gemini-2.5-pro', recognizedModelName: 'gemini-2.5-pro', ...geminiModelOptions['gemini-2.5-pro'] }
		if (lower.includes('2.5') && lower.includes('flash')) return { modelName: 'gemini-2.5-flash', recognizedModelName: 'gemini-2.5-flash', ...geminiModelOptions['gemini-2.5-flash'] }
		if (lower.includes('2.0') || lower.includes('2-0')) return { modelName: 'gemini-2.5-pro', recognizedModelName: 'gemini-2.5-pro', ...geminiModelOptions['gemini-2.5-pro'] } // map 2.0 to 2.5
		return null
	},
}
...
const deepseekSettings: raccoonStaticProviderInfo = {
...
const mistralSettings: raccoonStaticProviderInfo = {
...
const groqSettings: raccoonStaticProviderInfo = {
...
} as const satisfies Record<string, raccoonStaticModelInfo>
const googleVertexSettings: raccoonStaticProviderInfo = {
...
const microsoftAzureSettings: raccoonStaticProviderInfo = {
...
const awsBedrockSettings: raccoonStaticProviderInfo = {
...
const vLLMSettings: raccoonStaticProviderInfo = {
...
const lmStudioSettings: raccoonStaticProviderInfo = {
...
const ollamaSettings: raccoonStaticProviderInfo = {
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
...
const openaiCompatible: raccoonStaticProviderInfo = {
...
const liteLLMSettings: raccoonStaticProviderInfo = { // https://docs.litellm.ai/docs/reasoning_content
	modelOptionsFallback: (modelName) => extensiveModelOptionsFallback(modelName, { downloadable: { sizeGb: 'not-known' } }),
	modelOptions: {},
	providerReasoningIOSettings: {
		input: { includeInPayload: openAICompatIncludeInPayloadReasoning },
...
const openRouterSettings: raccoonStaticProviderInfo = {
...
const modelSettingsOfProvider: { [providerName in ProviderName]: raccoonStaticProviderInfo } = {
...
export const getModelCapabilities = (
	...
	const providerSettings = modelSettingsOfProvider[providerName]
	if (!providerSettings) {
		return { modelName, ...defaultModelOptions, ...overrides, isUnrecognizedModel: true };
	}

	const { modelOptions, modelOptionsFallback } = providerSettings
...
export const getProviderCapabilities = (providerName: ProviderName) => {
	const providerSettings = modelSettingsOfProvider[providerName]
	if (!providerSettings) {
		return { providerReasoningIOSettings: undefined }
	}
	const { providerReasoningIOSettings } = providerSettings
