...
import { RACCOON_SETTINGS_STORAGE_KEY } from './storageKeys.js';
import { defaultSettingsOfProvider, FeatureName, ModelSelectionOfFeature, SettingsOfProvider, SettingName, ModelSelection, modelSelectionsEqual, featureNames, RaccoonStatefulModelInfo, GlobalSettings, GlobalSettingName, defaultGlobalSettings, ModelSelectionOptions, OptionsOfModelSelection, ChatMode, OverridesOfModel, defaultOverridesOfModel, MCPUserStateOfName as MCPUserStateOfName, MCPUserState } from './raccoonSettingsTypes.js';
import { ProviderName, providerNames } from './raccoonConstants.js';
...
export type RaccoonSettingsState = {
...
// type RealRaccoonSettings = Exclude<keyof RaccoonSettingsState, '_modelOptions'>
// type EventProp<T extends RealRaccoonSettings = RealRaccoonSettings> = T extends 'globalSettings' ? [T, keyof RaccoonSettingsState[T]] : T | 'all'


export interface IRaccoonSettingsService {
	readonly _serviceBrand: undefined;
	readonly state: RaccoonSettingsState; // in order to play nicely with react, you should immutably change state
	...
	dangerousSetState(newState: RaccoonSettingsState): Promise<void>;
...
const _modelsWithSwappedInNewModels = (options: { existingModels: RaccoonStatefulModelInfo[], models: string[], type: 'autodetected' | 'default' }) => {
	const { existingModels, models, type } = options

	const existingModelsMap: Record<string, RaccoonStatefulModelInfo> = {}
...
const _stateWithMergedDefaultModels = (state: RaccoonSettingsState): RaccoonSettingsState => {
...
const _validatedModelState = (state: Omit<RaccoonSettingsState, '_modelOptions'>): RaccoonSettingsState => {
	...
	} satisfies RaccoonSettingsState
...
export const IRaccoonSettingsService = createDecorator<IRaccoonSettingsService>('RaccoonSettingsService');
class RaccoonSettingsService extends Disposable implements IRaccoonSettingsService {
	_serviceBrand: undefined;

	private readonly _onDidChangeState = new Emitter<void>();
	readonly onDidChangeState: Event<void> = this._onDidChangeState.event; // this is primarily for use in react, so react can listen + update on state changes

	state: RaccoonSettingsState;
	...
	dangerousSetState = async (newState: RaccoonSettingsState) => {
	...
	async readAndInitializeState() {
		let readS: RaccoonSettingsState
		try {
	...
	private async _readState(): Promise<RaccoonSettingsState> {
		const encryptedState = this._storageService.get(RACCOON_SETTINGS_STORAGE_KEY, StorageScope.APPLICATION)
	...
	private async _storeState() {
		...
		this._storageService.store(RACCOON_SETTINGS_STORAGE_KEY, encryptedState, StorageScope.APPLICATION, StorageTarget.USER);
	}
	...
	setGlobalSetting: SetGlobalSettingFn = async (settingName, newVal) => {
		const newState: RaccoonSettingsState = {
			...this.state,
			globalSettings: {
	...
	toggleModelHidden(providerName: ProviderName, modelName: string) {
		...
		const newModels: RaccoonStatefulModelInfo[] = [
...
registerSingleton(IRaccoonSettingsService, RaccoonSettingsService, InstantiationType.Eager);
