...
import { displayInfoOfProviderName } from './raccoonSettingsTypes.js';
...
export class LLMMessageService extends Disposable implements ILLMMessageService {
	...
	constructor(
		...
		this.channel = this.mainProcessService.getChannel('raccoon-channel-llmMessage')
	...
	sendLLMMessage(params: ServiceSendLLMMessageParams) {
		...
		if (modelSelection === null) {
			const message = `Please add a provider in Raccoon's Settings.`
		...
		const { settingsOfProvider, } = this.raccoonSettingsService.state
		const selectedProviderName = modelSelection.providerName as ProviderName
		const settingsAtProvider = settingsOfProvider[selectedProviderName]
		if (!settingsAtProvider?._didFillInProviderSettings) {
			let providerTitle: string = selectedProviderName
			try {
				providerTitle = displayInfoOfProviderName(selectedProviderName).title
			} catch {
				// keep fallback
			}
			const message = `Please add your ${providerTitle} settings in Raccoon's Settings.`
			onError({ message, fullError: null })
			return null
		}
