...
import { IRaccoonSCMService } from '../common/raccoonSCMTypes.js'
import { IMainProcessService } from '../../../../platform/ipc/common/mainProcessService.js'
import { IRaccoonSettingsService } from '../common/raccoonSettingsService.js'
import { IConvertToLLMMessageService } from './convertToLLMMessageService.js'
import { ILLMMessageService } from '../common/sendLLMMessageService.js'
import { ModelSelection, OverridesOfModel, ModelSelectionOptions } from '../common/raccoonSettingsTypes.js'
...
export const IGenerateCommitMessageService = createDecorator<IGenerateCommitMessageService>('raccoonGenerateCommitMessageService');

const loadingContextKey = 'raccoonSCMGenerateCommitMessageLoading'
...
class GenerateCommitMessageService extends Disposable implements IGenerateCommitMessageService {
	...
	private raccoonSCM: IRaccoonSCMService
	private loadingContextKey: IContextKey<boolean>

	constructor(
		@ISCMService private readonly scmService: ISCMService,
		@IMainProcessService mainProcessService: IMainProcessService,
		@IRaccoonSettingsService private readonly raccoonSettingsService: IRaccoonSettingsService,
		...
		this.raccoonSCM = ProxyChannel.toService<IRaccoonSCMService>(mainProcessService.getChannel('raccoon-channel-scm'))
	}
		...
		this.execute.trigger(async () => {
			...
			try {
				...
				const [stat, sampledDiffs, branch, log] = await Promise.all([
					this.raccoonSCM.gitStat(path),
					this.raccoonSCM.gitSampledDiffs(path),
					this.raccoonSCM.gitBranch(path),
					this.raccoonSCM.gitLog(path)
				])

				if (!this.isCurrentRequest(requestId)) { throw new CancellationError() }

				const modelSelection = this.raccoonSettingsService.state.modelSelectionOfFeature['SCM'] ?? null
				const modelSelectionOptions = modelSelection ? this.raccoonSettingsService.state.optionsOfModelSelection['SCM'][modelSelection?.providerName]?.[modelSelection.modelName] : undefined
				const overridesOfModel = this.raccoonSettingsService.state.overridesOfModel
	...
	private sendLLMMessage(messages: LLMChatMessage[], separateSystemMessage: string, modelOptions: ModelOptions): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			this.llmRequestId = this.llmMessageService.sendLLMMessage({
				...
				logging: { loggingName: 'raccoonSCM - Commit Message' },
			})
		})
	}
	...
	private onError(error: any) {
		if (!isCancellationError(error)) {
			console.error(error)
			this.notificationService.error(localize2('raccoonFailedToGenerateCommitMessage', 'Failed to generate commit message.').value)
		}
	}
}
...
class GenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'raccoon.generateCommitMessageAction',
			title: localize2('raccoonCommitMessagePrompt', 'Raccoon AI: Generate Commit Message'),
			icon: ThemeIcon.fromId('sparkle'),
			tooltip: localize2('raccoonCommitMessagePromptTooltip', 'Raccoon AI: Generate Commit Message'),
			f1: true,
			menu: [{
...
class LoadingGenerateCommitMessageAction extends Action2 {
	constructor() {
		super({
			id: 'raccoon.loadingGenerateCommitMessageAction',
			title: localize2('raccoonCommitMessagePromptCancel', 'Raccoon AI: Cancel Commit Message Generation'),
			icon: ThemeIcon.fromId('stop-circle'),
			tooltip: localize2('raccoonCommitMessagePromptCancelTooltip', 'Raccoon AI: Cancel Commit Message Generation'),
