...
import { raccoonPrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IRaccoonCommandBarService } from './raccoonCommandBarService.js';
import { IKeybindingService } from '../../../../platform/keybinding/common/keybinding.js';
import { raccoon_ACCEPT_DIFF_ACTION_ID, raccoon_REJECT_DIFF_ACTION_ID } from './actionIDs.js';
...
import { IRaccoonSettingsService } from '../common/raccoonSettingsService.js';
import { FeatureName } from '../common/raccoonSettingsTypes.js';
import { IRaccoonModelService } from '../common/raccoonModelService.js';
import { deepClone } from '../../../../base/common/objects.js';
import { acceptBg, acceptBorder, buttonFontSize, buttonTextColor, rejectBg, rejectBorder } from '../common/helpers/colors.js';
import { DiffArea, Diff, CtrlKZone, raccoonFileSnapshot, DiffAreaSnapshotEntry, diffAreaSnapshotKeys, DiffZone, TrackingZone, ComputedDiff } from '../common/editCodeServiceTypes.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IRaccoonSettingsService private readonly _settingsService: IRaccoonSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IRaccoonModelService private readonly _raccoonModelService: IRaccoonModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._raccoonModelService.initializeModel(model.uri)
	...
	// 		message: `Raccoon Error: ${e.message}`,
	// 		actions: {
	// 			secondary: [{
	// 				id: 'raccoon.onerror.opensettings',
	// 				enabled: true,
	// 				label: `Open Raccoon's settings`,
	// 				tooltip: '',
	// 				class: undefined,
	// 				run: () => { this._commandService.executeCommand(raccoon_OPEN_SETTINGS_ACTION_ID) }
	// 			}]
	// 		},
	// 		source: details ? `(Hold ${isMacintosh ? 'Option' : 'Alt'} to hover) - ${details}\n\nIf this persists, feel free to [report](https://github.com/DevFlex-AI/Raccoon/issues/new) it.` : undefined
	// 	})
	// }
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._raccoonModelService.getModel(uri)
		...
		for (const diffareaid of this.diffAreasOfURI[uri.fsPath] || []) {
			...
			if (diffArea.type === 'DiffZone') {
				...
				if (diffArea._streamState.isStreaming) {
					// sweepLine ... sweepLine
					const fn1 = this._addLineDecoration(model, diffArea._streamState.line, diffArea._streamState.line, 'raccoon-sweepIdxBG')
					// sweepLine+1 ... endLine
					const fn2 = diffArea._streamState.line + 1 <= diffArea.endLine ?
						this._addLineDecoration(model, diffArea._streamState.line + 1, diffArea.endLine, 'raccoon-sweepBG')
						: null
				...
				const fn = this._addLineDecoration(model, diffArea.startLine, diffArea.endLine, 'raccoon-highlightBG')
		...
		if (type !== 'insertion') {
			const consistentZoneId = this._consistentItemService.addConsistentItemToURI({
				...
				fn: (editor) => {
					...
					const domNode = document.createElement('div');
					domNode.className = 'raccoon-redBG'
	...
	private _getCurrentraccoonFileSnapshot = (uri: URI): raccoonFileSnapshot => {
		const { model } = this._raccoonModelService.getModel(uri)
	...
	private _restoreraccoonFileSnapshot = async (uri: URI, snapshot: raccoonFileSnapshot) => {
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		const beforeSnapshot: raccoonFileSnapshot = this._getCurrentraccoonFileSnapshot(uri)
		let afterSnapshot: raccoonFileSnapshot | null = null
		...
		const elt: IUndoRedoElement = {
			...
			undo: async () => { opts?.onWillUndo?.(); await this._restoreraccoonFileSnapshot(uri, beforeSnapshot) },
			redo: async () => { if (afterSnapshot) await this._restoreraccoonFileSnapshot(uri, afterSnapshot) }
		}
		this._undoRedoService.pushElement(elt)

		const onFinishEdit = async () => {
			afterSnapshot = this._getCurrentraccoonFileSnapshot(uri)
			await this._raccoonModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}


	public getraccoonFileSnapshot(uri: URI) {
		return this._getCurrentraccoonFileSnapshot(uri)
	}


	public restoreraccoonFileSnapshot(uri: URI, snapshot: raccoonFileSnapshot): void {
		this._restoreraccoonFileSnapshot(uri, snapshot)
	}
	...
	private _writeStreamedDiffZoneLLMText(uri: URI, originalCode: string, llmTextSoFar: string, deltaText: string, latestMutable: StreamLocationMutable) {
		...
		if (!lastDiff) {
			...
			if (lastDiff.type === 'insertion' || lastDiff.type === 'edit')
				...
				throw new Error(`Raccoon AI: diff.type not recognized on: ${lastDiff}`)
		}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._raccoonModelService.initializeModel(uri)
		await this._raccoonModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			throw new Error(`Raccoon AI: diff.type not recognized on: ${from}`)
		}
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = raccoonPrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
			...
			while (shouldSendAnotherMessage) {
				...
				streamRequestIdRef.current = this._llmMessageService.sendLLMMessage({
					...
					onFinalMessage: async (params) => {
						...
						const blocks = extractSearchReplaceBlocks(fullText)
						if (blocks.length === 0) {
							this._notificationService.info(`Raccoon: We ran Fast Apply, but the LLM didn't output any changes.`)
						}
						this._writeURIText(uri, originalFileCode, 'wholeFileRange', { shouldRealignDiffAreas: true })
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IRaccoonCommandBarService private readonly _raccoonCommandBarService: IRaccoonCommandBarService,
		...
		const getAcceptRejectText = () => {
			const acceptKeybinding = this._keybindingService.lookupKeybinding(raccoon_ACCEPT_DIFF_ACTION_ID);
			const rejectKeybinding = this._keybindingService.lookupKeybinding(raccoon_REJECT_DIFF_ACTION_ID);
			...
			const commandBarStateAtUri = this._raccoonCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._raccoonCommandBarService.onDidChangeState(e => {
