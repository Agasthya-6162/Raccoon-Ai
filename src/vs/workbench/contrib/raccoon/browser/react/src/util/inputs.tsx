...
export const WidgetComponent = <CtorParams extends any[], Instance>({ ctor, propsFn, dispose, onCreateInstance, children, className
  ...
  return <div ref={containerRef} className={className === undefined ? `raccoon-w-full` : className}>{children}</div>;
};
...
export const RaccoonInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
  ...
  return <>
      ...
      className={`raccoon-w-full raccoon-resize-none raccoon-max-h-[500px] raccoon-overflow-y-auto raccoon-text-raccoon-fg-1 placeholder:raccoon-text-raccoon-fg-3 ${className}`}
      style={{
    ...
    <div
      ...
      className="raccoon-z-[100] raccoon-border-raccoon-border-3 raccoon-bg-raccoon-bg-2-alt raccoon-border raccoon-rounded raccoon-shadow-lg raccoon-flex raccoon-flex-col raccoon-overflow-hidden"
      style={{
        position: strategy,
        top: y ?? 0,
        left: x ?? 0,
      ...
      onWheel={(e) => e.stopPropagation()}>
				...
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="raccoon-px-2 raccoon-py-1 raccoon-text-raccoon-fg-1 raccoon-border-b raccoon-border-raccoon-border-3 raccoon-sticky raccoon-top-0 raccoon-bg-raccoon-bg-1 raccoon-z-10 raccoon-select-none raccoon-pointer-events-none">
					{optionText ?
        ...
        <div className="raccoon-opacity-50">Enter text to filter...</div>
        }
				</div>}


				{/* Options list */}
				<div className="raccoon-max-h-[400px] raccoon-w-full raccoon-max-w-full raccoon-overflow-y-auto raccoon-overflow-x-auto">
					<div className="raccoon-w-max raccoon-min-w-full raccoon-flex raccoon-flex-col raccoon-gap-0 raccoon-text-nowrap raccoon-flex-nowrap">
						{options.length === 0 ?
          <div className="raccoon-text-raccoon-fg-3 raccoon-px-3 raccoon-py-0.5">No results found</div> :
          options.map((o, oIdx) => {
            ...
            return (
              ...
              <div
                ...
                key={o.fullName}
                className={` raccoon-flex raccoon-items-center raccoon-gap-2 raccoon-px-3 raccoon-py-1 raccoon-cursor-pointer ${


                oIdx === optionIdx ? "raccoon-bg-blue-500 raccoon-text-white/80" : "raccoon-bg-raccoon-bg-2-alt raccoon-text-raccoon-fg-1"} `}
                ...
                onMouseMove={() => {setOptionIdx(oIdx);}}>
										...
										{o.fullName && o.fullName !== o.abbreviatedName && <span className="raccoon-opacity-60 raccoon-text-sm">{o.fullName}</span>}
...
export const RaccoonSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps
  ...
  return (
    <input
      ...
      placeholder={placeholder}
      disabled={disabled}
      className={`raccoon-w-full raccoon-resize-none raccoon-bg-raccoon-bg-1 raccoon-text-raccoon-fg-1 placeholder:raccoon-text-raccoon-fg-3 raccoon-border raccoon-border-raccoon-border-2 focus:raccoon-border-raccoon-border-1 ${
      compact ? "raccoon-py-1 raccoon-px-2" : "raccoon-py-2 raccoon-px-4 "} raccoon-rounded ${

      disabled ? "raccoon-opacity-50 raccoon-cursor-not-allowed" : ""} ${
      className}`}
      style={{
...
export const RaccoonInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline
  ...
  return <WidgetComponent
    className=" raccoon-bg-raccoon-bg-1 raccoon-force-child-placeholder-raccoon-fg-1 "
...
export const RaccoonSlider = ({
  ...
  return (
    <div className={`raccoon-inline-flex raccoon-items-center raccoon-flex-shrink-0 ${className}`}>
			{/* Outer container with padding to account for thumb overhang */}
			<div className={`raccoon-relative raccoon-flex-shrink-0 ${disabled ? "raccoon-opacity-25" : ""}`}
      style={{
        width
        ...
        // paddingRight: thumbSizePx / 2
				...
				<div className="raccoon-relative raccoon-w-full">
					{/* Invisible wider clickable area that sits above the track */}
					<div
            className="raccoon-absolute raccoon-w-full raccoon-cursor-pointer"
            style={{
              height: '16px',
              top: '50%',
            ...
            onClick={handleTrackClick} />
					...
					{/* Track */}
					<div
            className={`raccoon-relative ${size === 'xxs' ? "raccoon-h-0.5" :
            size === 'xs' ? "raccoon-h-1" :
            size === 'sm' ? "raccoon-h-1.5" :
            size === 'sm+' ? "raccoon-h-2" : "raccoon-h-2.5"} raccoon-bg-raccoon-bg-2 raccoon-rounded-full raccoon-cursor-pointer`}

            onClick={handleTrackClick}>

						{/* Filled part of track */}
						<div
              className={`raccoon-absolute raccoon-left-0 ${size === 'xxs' ? "raccoon-h-0.5" :
              size === 'xs' ? "raccoon-h-1" :
              size === 'sm' ? "raccoon-h-1.5" :
              size === 'sm+' ? "raccoon-h-2" : "raccoon-h-2.5"} raccoon-bg-raccoon-fg-1 raccoon-rounded-full`}
              ...
              style={{ width: `${percentage}%` }} />
					...
					{/* Thumb */}
					<div
            className={`raccoon-absolute raccoon-top-1/2 raccoon-transform -raccoon-translate-x-1/2 -raccoon-translate-y-1/2 ${
            size === 'xxs' ? "raccoon-h-2 raccoon-w-2" :
            size === 'xs' ? "raccoon-h-2.5 raccoon-w-2.5" :
            size === 'sm' ? "raccoon-h-3 raccoon-w-3" :
            size === 'sm+' ? "raccoon-h-3.5 raccoon-w-3.5" : "raccoon-h-4 raccoon-w-4"} raccoon-bg-raccoon-fg-1 raccoon-rounded-full raccoon-shadow-md ${

            disabled ? "raccoon-cursor-not-allowed" : "raccoon-cursor-grab active:raccoon-cursor-grabbing"} raccoon-border raccoon-border-raccoon-fg-1`}
...
export const RaccoonSwitch = ({
  ...
  return (
    <label className="raccoon-inline-flex raccoon-items-center" {...props}>
			<div
        onClick={() => !disabled && onChange(!value)}
        className={` raccoon-cursor-pointer raccoon-relative raccoon-inline-flex raccoon-items-center raccoon-rounded-full raccoon-transition-colors raccoon-duration-200 raccoon-ease-in-out ${


        value ? "raccoon-bg-zinc-900 dark:raccoon-bg-white" : "raccoon-bg-white dark:raccoon-bg-zinc-600"} ${
        disabled ? "raccoon-opacity-25" : ""} ${
        size === 'xxs' ? "raccoon-h-3 raccoon-w-5" : ""} ${
        size === 'xs' ? "raccoon-h-4 raccoon-w-7" : ""} ${
        size === 'sm' ? "raccoon-h-5 raccoon-w-9" : ""} ${
        size === 'sm+' ? "raccoon-h-5 raccoon-w-10" : ""} ${
        size === 'md' ? "raccoon-h-6 raccoon-w-11" : ""} `}>


				<span
          className={` raccoon-inline-block raccoon-transform raccoon-rounded-full raccoon-bg-white dark:raccoon-bg-zinc-900 raccoon-shadow raccoon-transition-transform raccoon-duration-200 raccoon-ease-in-out ${

          size === 'xxs' ? "raccoon-h-2 raccoon-w-2" : ""} ${
          size === 'xs' ? "raccoon-h-2.5 raccoon-w-2.5" : ""} ${
          size === 'sm' ? "raccoon-h-3 raccoon-w-3" : ""} ${
          size === 'sm+' ? "raccoon-h-3.5 raccoon-w-3.5" : ""} ${
          size === 'md' ? "raccoon-h-4 raccoon-w-4" : ""} ${
          size === 'xxs' ? value ? "raccoon-translate-x-2.5" : "raccoon-translate-x-0.5" : ""} ${
          size === 'xs' ? value ? "raccoon-translate-x-3.5" : "raccoon-translate-x-0.5" : ""} ${
          size === 'sm' ? value ? "raccoon-translate-x-5" : "raccoon-translate-x-1" : ""} ${
          size === 'sm+' ? value ? "raccoon-translate-x-6" : "raccoon-translate-x-1" : ""} ${
          size === 'md' ? value ? "raccoon-translate-x-6" : "raccoon-translate-x-1" : ""} `} />
...
export const RaccoonCheckbox = ({ label, value, onClick, className }: {label: string;value: boolean;onClick: (checked: boolean) => void;className?: string;}) => {
...
export const RaccoonCustomDropdownBox = <T extends NonNullable<any>,>({
  options,
  selectedOption,
  onChangeOption,
  ...
  return (
    <div className={`raccoon-inline-block raccoon-relative ${className}`}>
			{/* Hidden measurement div */}
			<div
        ref={measureRef}
        className="raccoon-opacity-0 raccoon-pointer-events-none raccoon-absolute -raccoon-left-[999999px] -raccoon-top-[999999px] raccoon-flex raccoon-flex-col"
        aria-hidden="true">
          ...
          return (
            <div key={optionName + optionDetail} className="raccoon-flex raccoon-items-center raccoon-whitespace-nowrap">
							<div className="raccoon-w-4" />
							<span className="raccoon-flex raccoon-justify-between raccoon-w-full">
        ...
        className="raccoon-flex raccoon-items-center raccoon-h-4 raccoon-bg-transparent raccoon-whitespace-nowrap hover:raccoon-brightness-90 raccoon-w-full"
        onClick={() => setIsOpen(!isOpen)}>

				<span className={`raccoon-truncate ${arrowTouchesText ? "raccoon-mr-1" : ""}`}>
					{getOptionDisplayName(selectedOption)}
				</span>
				<svg
          className={`raccoon-size-3 raccoon-flex-shrink-0 ${arrowTouchesText ? "" : "raccoon-ml-auto"}`}
          viewBox="0 0 12 12"
          fill="none">
      ...
      <div
        ref={refs.setFloating}
        className="raccoon-z-[100] raccoon-bg-raccoon-bg-1 raccoon-border-raccoon-border-3 raccoon-border raccoon-rounded raccoon-shadow-lg"
        style={{
          position: strategy,
          top: y ?? 0,
          left: x ?? 0,
        ...
        onWheel={(e) => e.stopPropagation()}>
        <div className="raccoon-overflow-auto raccoon-max-h-80">
            ...
            return (
              <div
                key={optionName}
                className={`raccoon-flex raccoon-items-center raccoon-px-2 raccoon-py-1 raccoon-pr-4 raccoon-cursor-pointer raccoon-whitespace-nowrap raccoon-transition-all raccoon-duration-100 ${

                thisOptionIsSelected ? "raccoon-bg-blue-500 raccoon-text-white/80" : "hover:raccoon-bg-blue-500 hover:raccoon-text-white/80"} `}
                ...
                onClick={() => {
                  ...
                  setIsOpen(false);
									...
									<div className="raccoon-w-4 raccoon-flex raccoon-justify-center raccoon-flex-shrink-0">
										{thisOptionIsSelected &&
                  <svg className="raccoon-size-3" viewBox="0 0 12 12" fill="none">
												<path
                      d="M10 3L4.5 8.5L2 6"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      ...
                      strokeLinejoin="round" />
									...
									<span className="raccoon-flex raccoon-justify-between raccoon-items-center raccoon-w-full raccoon-gap-x-1">
										<span>{optionName}</span>
										<span className="raccoon-opacity-60">{optionDetail}</span>
									</span>
								</div>);
...
export const _raccoonSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className
  ...
  return <WidgetComponent
    className={` select-child-restyle [&_select]:!raccoon-text-raccoon-fg-3 [&_select]:!raccoon-text-xs !raccoon-text-raccoon-fg-3 ${
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
  ...
  return <div ref={divRef} className="raccoon-relative raccoon-z-0 raccoon-px-2 raccoon-py-1 raccoon-bg-raccoon-bg-3">
		<WidgetComponent
...
export const RaccoonButtonBgDarken = ({ children, disabled, onClick, className }: {children: React.ReactNode;disabled?: boolean;onClick: () => void;className?: string;}) => {
  return <button disabled={disabled}
  className={`raccoon-px-3 raccoon-py-1 raccoon-bg-black/10 dark:raccoon-bg-white/10 raccoon-rounded-sm raccoon-overflow-hidden raccoon-whitespace-nowrap raccoon-flex raccoon-items-center raccoon-justify-center ${className || ''}`}
  onClick={onClick}>
    {children}</button>;
};

// export const raccoonScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const raccoonSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
...
// export const RaccoonCheckbox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: {block: ExtractedSearchReplaceBlock;lang: string | undefined;}) => {
  ...
  return (
    <div className="raccoon-w-full raccoon-bg-raccoon-bg-3 bg-editor-style-override" ref={divRef} />);
 ...
 */
export const RaccoonDiffEditor = ({ uri, searchReplaceBlocks, language }: {uri?: any;searchReplaceBlocks: string;language?: string;}) => {
  const accessor = useAccessor();
  const languageService = accessor.get('ILanguageService');
  ...
  if (blocks.length === 0) {
    return <div className="raccoon-w-full raccoon-p-4 raccoon-text-raccoon-fg-4 raccoon-text-sm">No changes found</div>;
  }

  // Display all blocks
  return (
    <div className="raccoon-w-full raccoon-flex raccoon-flex-col raccoon-gap-2">
			{blocks.map((block, index) =>
      <div key={index} className="raccoon-w-full">
					{blocks.length > 1 &&
        <div className="raccoon-text-raccoon-fg-4 raccoon-text-xs raccoon-mb-1 raccoon-px-1">
