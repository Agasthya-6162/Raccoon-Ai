...
import { FeatureName, featureNames, isFeatureNameDisabled, ModelSelection, modelSelectionsEqual, ProviderName, providerNames, SettingsOfProvider } from '../../../../../../../workbench/contrib/raccoon/common/raccoonSettingsTypes.js';
import { useSettingsState, useRefreshModelState, useAccessor } from '../util/services.js';
import { _raccoonSelectBox, RaccoonCustomDropdownBox } from '../util/inputs.js';
import { SelectBox } from '../../../../../../../base/browser/ui/selectBox/selectBox.js';
import { IconWarning } from '../sidebar-tsx/SidebarChat.js';
import { raccoon_OPEN_SETTINGS_ACTION_ID, raccoon_TOGGLE_SETTINGS_ACTION_ID } from '../../../raccoonSettingsPane.js';
import { modelFilterOfFeatureName, ModelOption } from '../../../../../../../workbench/contrib/raccoon/common/raccoonSettingsService.js';
...
const ModelSelectBox = ({ options, featureName, className }: {options: ModelOption[];featureName: FeatureName;className: string;}) => {
  ...
  const selection = raccoonSettingsService.state.modelSelectionOfFeature[featureName];
  const selectedOption = selection
    ? raccoonSettingsService.state._modelOptions.find((v) => modelSelectionsEqual(v.selection, selection))
    : undefined;
  const effectiveOption = selectedOption ?? options[0];

  useEffect(() => {
    if (!selectedOption && options[0]) {
      raccoonSettingsService.setModelSelectionOfFeature(featureName, options[0].selection);
    }
  }, [selectedOption, options, raccoonSettingsService, featureName]);

  const onChangeOption = useCallback((newOption: ModelOption) => {
    raccoonSettingsService.setModelSelectionOfFeature(featureName, newOption.selection);
  }, [raccoonSettingsService, featureName]);
...
export const ModelDropdown = ({ featureName, className }: {featureName: FeatureName;className: string;}) => {
  ...
  const openSettings = () => {commandService.executeCommand(raccoon_OPEN_SETTINGS_ACTION_ID);};
