...
import { isFeatureNameDisabled } from '../../../../common/raccoonSettingsTypes.js';
import { URI } from '../../../../../../../base/common/uri.js';
import { FileSymlink, LucideIcon, RotateCw, Terminal } from 'lucide-react';
import { Check, X, Square, Copy, Play } from 'lucide-react';
import { getBasename, ListableToolItem, raccoonOpenFileFn, ToolChildrenWrapper } from '../sidebar-tsx/SidebarChat.js';
...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return <button
    ...
    // border border-raccoon-border-1 rounded
    className={` raccoon-size-[18px] raccoon-p-[2px] raccoon-flex raccoon-items-center raccoon-justify-center raccoon-text-sm raccoon-text-raccoon-fg-3 hover:raccoon-brightness-110 disabled:raccoon-opacity-50 disabled:raccoon-cursor-not-allowed ${
...
export const JumpToFileButton = ({ uri, ...props }: {uri: URI | 'current';} & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  <IconShell1
    Icon={FileSymlink}
    onClick={() => {
      raccoonOpenFileFn(uri, accessor);
    }}
...
export const useApplyStreamState = ({ applyBoxId }: {applyBoxId: string;}) => {
  const accessor = useAccessor();
  const raccoonCommandBarService = accessor.get('IRaccoonCommandBarService');

  const getStreamState = useCallback(() => {
    const uri = getUriBeingApplied(applyBoxId);
    if (!uri) return 'idle-no-changes';
    return raccoonCommandBarService.getStreamState(uri);
  }, [raccoonCommandBarService, applyBoxId]);
...
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={`raccoon-flex raccoon-flex-row raccoon-text-raccoon-fg-3 raccoon-text-xs raccoon-items-center raccoon-gap-1.5 ${className}`} {...props}>
			{title && <span className="raccoon-opacity-80">{title}</span>}
			<div
        className={` raccoon-size-1.5 raccoon-rounded-full raccoon-border ${
        indicatorColor === 'dark' ? "raccoon-bg-[rgba(0,0,0,0)] raccoon-border-raccoon-border-1" :
        indicatorColor === 'orange' ? "raccoon-bg-orange-500 raccoon-border-orange-500 raccoon-shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]" :
        indicatorColor === 'green' ? "raccoon-bg-green-500 raccoon-border-green-500 raccoon-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" :
        indicatorColor === 'yellow' ? "raccoon-bg-yellow-500 raccoon-border-yellow-500 raccoon-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" : "raccoon-bg-raccoon-border-1 raccoon-border-raccoon-border-1"} `} />
...
const tooltipPropsForApplyBlock = ({ tooltipName, color = undefined, position = 'top', offset = undefined }: {tooltipName: string;color?: IndicatorColor;position?: PlacesType;offset?: number;}) => ({
  'data-tooltip-id': color === 'orange' ? `raccoon-tooltip-orange` : color === 'green' ? 'raccoon-tooltip-green' : 'raccoon-tooltip',
...
export const useEditToolStreamState = ({ applyBoxId, uri }: {applyBoxId: string;uri: URI;}) => {
  const accessor = useAccessor();
  const raccoonCommandBarService = accessor.get('IRaccoonCommandBarService');
  const [streamState, setStreamState] = useState(raccoonCommandBarService.getStreamState(uri));
  // listen for stream updates on this box
  useCommandBarURIListener(useCallback((uri_) => {
    const shouldUpdate = uri.fsPath === uri_.fsPath;
    if (shouldUpdate) {setStreamState(raccoonCommandBarService.getStreamState(uri));}
  }, [raccoonCommandBarService, applyBoxId, uri]));
...
const ApplyButtonsForEdit = ({
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(`Raccoon Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`);
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`Raccoon Error: There was a problem running Apply: ${e}.`);
...
export const BlockCodeApplyWrapper = ({
  ...
  <ListableToolItem
    name={<span className="raccoon-not-italic">{getBasename(uri.fsPath)}</span>}
    isSmall={true}
    showDot={false}
  ...
  return <div className="raccoon-border raccoon-border-raccoon-border-3 raccoon-rounded raccoon-overflow-hidden raccoon-bg-raccoon-bg-3 raccoon-my-1">
		{/* header */}
		<div className=" raccoon-select-none raccoon-flex raccoon-justify-between raccoon-items-center raccoon-py-1 raccoon-px-2 raccoon-border-b raccoon-border-raccoon-border-3 raccoon-cursor-default">
			<div className="raccoon-flex raccoon-items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="raccoon-text-[13px] raccoon-font-light raccoon-text-raccoon-fg-3">
					{name}
				</span>
			</div>
			<div className={`${canApply ? "" : "raccoon-hidden"} raccoon-flex raccoon-items-center raccoon-gap-1`}>
