...
import { getBasename, getRelative, raccoonOpenFileFn } from '../sidebar-tsx/SidebarChat.js';
...
const LatexRender = ({ latex }: {latex: string;}) => {
  return <span className="raccoon-katex-error raccoon-text-red-500">{latex}</span>;
  // try {
  // 	let formula = latex;
...
const Codespan = ({ text, className, onClick, tooltip }: {text: string;className?: string;onClick?: () => void;tooltip?: string;}) => {
  ...
  return <code
    className={`raccoon-font-mono raccoon-font-medium raccoon-rounded-sm raccoon-bg-raccoon-bg-1 raccoon-px-1 ${className}`}
    onClick={onClick}
    {...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  const onClick = () => {
    if (!link) return;
    // Use the updated raccoonOpenFileFn to open the file and handle selection
    if (link.selection)
    raccoonOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);else

    raccoonOpenFileFn(link.uri, accessor);
  };

  return <Codespan
    text={displayText}
    onClick={onClick}
    className={link ? "raccoon-underline hover:raccoon-brightness-90 raccoon-transition-all raccoon-duration-200 raccoon-cursor-pointer" : ""}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  ...
  if (t.type === 'table') {
    ...
    // return (
    // 	<div>
    // 		<table className={'min-w-full border border-raccoon-bg-2'}>
    // 			<thead>
    // 				<tr className='bg-raccoon-bg-1'>
    // 					{t.header.map((cell: any, index: number) => (
    // 						<th
    // 							key={index}
    // 							className='px-4 py-2 border border-raccoon-bg-2 font-semibold'
    ...
    // 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-raccoon-bg-1'}>
    // 						{row.map((cell: any, cellIndex: number) => (
    // 							<td
    // 								key={cellIndex}
    // 								className={'px-4 py-2 border border-raccoon-bg-2'}
  ...
  if (t.type === 'paragraph') {
    ...
    if (latexSegments.length !== 0) {
      if (inPTag) {
        return <span className="raccoon-block">{latexSegments}</span>;
      }
    ...
    if (inPTag) return <span className="raccoon-block">{contents}</span>;
    return <p>{contents}</p>;
  }
  ...
  if (t.type === 'link') {
    return (
      <a
        ...
        className="raccoon-underline raccoon-cursor-pointer hover:raccoon-brightness-90 raccoon-transition-all raccoon-duration-200 raccoon-text-raccoon-fg-2">
  ...
  // default
  return (
    <div className="raccoon-bg-orange-50 raccoon-rounded-sm raccoon-overflow-hidden raccoon-p-2">
			<span className="raccoon-text-sm raccoon-text-orange-500">Unknown token rendered...</span>
		</div>);
