...
import { mountraccoonCommandBar } from './react/out/raccoon-editor-widgets-tsx/index.js'
...
import { raccoon_ACCEPT_DIFF_ACTION_ID, raccoon_REJECT_DIFF_ACTION_ID, raccoon_GOTO_NEXT_DIFF_ACTION_ID, raccoon_GOTO_PREV_DIFF_ACTION_ID, raccoon_GOTO_NEXT_URI_ACTION_ID, raccoon_GOTO_PREV_URI_ACTION_ID, raccoon_ACCEPT_FILE_ACTION_ID, raccoon_REJECT_FILE_ACTION_ID, raccoon_ACCEPT_ALL_DIFFS_ACTION_ID, raccoon_REJECT_ALL_DIFFS_ACTION_ID } from './actionIDs.js';
...
export interface IRaccoonCommandBarService {
...
export const IRaccoonCommandBarService = createDecorator<IRaccoonCommandBarService>('raccoonCommandBarService');
...
export class RaccoonCommandBarService extends Disposable implements IRaccoonCommandBarService {
	_serviceBrand: undefined;

	static readonly ID = 'raccoon.commandBarService'
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._raccoonModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IRaccoonCommandBarService, RaccoonCommandBarService, InstantiationType.Delayed); // delayed is needed here :(


export type raccoonCommandBarProps = {
	uri: URI | null;
	editor: ICodeEditor;
}
...
class AcceptRejectAllFloatingWidget extends Widget implements IOverlayWidget {
	...
	constructor({ editor }: { editor: ICodeEditor, },
		...
		this.instantiationService.invokeFunction(accessor => {
			...
			const res = mountraccoonCommandBar(root, accessor, { uri, editor } satisfies raccoonCommandBarProps)
			if (!res) return
			this._register(toDisposable(() => res.dispose?.()))
			this._register(editor.onWillChangeModel((model) => {
				const uri = model.newModelUrl
				res.rerender({ uri, editor } satisfies raccoonCommandBarProps)
			}))
		})
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_ACCEPT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('raccoonAcceptDiffAction', 'Raccoon AI: Accept Diff'),
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Alt | KeyMod.Shift | KeyCode.Enter,
				mac: { primary: KeyMod.WinCtrl | KeyMod.Alt | KeyCode.Enter },
				weight: KeybindingWeight.raccoonExtension,
			}
		});
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_REJECT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('raccoonRejectDiffAction', 'Raccoon AI: Reject Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_GOTO_NEXT_DIFF_ACTION_ID,
			f1: true,
			title: localize2('raccoonGoToNextDiffAction', 'Raccoon AI: Go to Next Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_GOTO_PREV_DIFF_ACTION_ID,
			f1: true,
			title: localize2('raccoonGoToPrevDiffAction', 'Raccoon AI: Go to Previous Diff'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_GOTO_NEXT_URI_ACTION_ID,
			f1: true,
			title: localize2('raccoonGoToNextUriAction', 'Raccoon AI: Go to Next File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_GOTO_PREV_URI_ACTION_ID,
			f1: true,
			title: localize2('raccoonGoToPrevUriAction', 'Raccoon AI: Go to Previous File with Diffs'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_ACCEPT_FILE_ACTION_ID,
			f1: true,
			title: localize2('raccoonAcceptFileAction', 'Raccoon AI: Accept All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_REJECT_FILE_ACTION_ID,
			f1: true,
			title: localize2('raccoonRejectFileAction', 'Raccoon AI: Reject All Diffs in Current File'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_ACCEPT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('raccoonAcceptAllDiffsAction', 'Raccoon AI: Accept All Diffs in All Files'),
			keybinding: {
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_REJECT_ALL_DIFFS_ACTION_ID,
			f1: true,
			title: localize2('raccoonRejectAllDiffsAction', 'Raccoon AI: Reject All Diffs in All Files'),
			keybinding: {
