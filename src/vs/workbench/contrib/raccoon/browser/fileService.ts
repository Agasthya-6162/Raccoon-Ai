...
class FilePromptActionService extends Action2 {
	private static readonly raccoon_COPY_FILE_PROMPT_ID = 'raccoon.copyfileprompt'

	constructor() {
		super({
			id: FilePromptActionService.raccoon_COPY_FILE_PROMPT_ID,
			title: localize2('raccoonCopyPrompt', 'Raccoon: Copy Prompt'),
			menu: [{
		...
		try {
			...
			if (stat.isFile) {
				m = await messageOfSelection({
					type: 'File',
					uri,
					language: (await raccoonModelService.getModelSafe(uri)).model?.getLanguageId() || '',
