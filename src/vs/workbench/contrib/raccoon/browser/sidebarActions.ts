...
import { raccoon_VIEW_CONTAINER_ID, raccoon_VIEW_ID } from './sidebarPane.js';
import { IMetricsService } from '../common/metricsService.js';
import { ICommandService } from '../../../../platform/commands/common/commands.js';
import { raccoon_TOGGLE_SETTINGS_ACTION_ID } from './raccoonSettingsPane.js';
...
const raccoon_OPEN_SIDEBAR_ACTION_ID = 'raccoon.sidebar.open'
registerAction2(class extends Action2 {
	constructor() {
		super({ id: raccoon_OPEN_SIDEBAR_ACTION_ID, title: localize2('raccoonOpenSidebar', 'Raccoon AI: Open Sidebar'), f1: true });
	}
	async run(accessor: ServicesAccessor): Promise<void> {
		const viewsService = accessor.get(IViewsService)
		const chatThreadsService = accessor.get(IChatThreadService)
		viewsService.openViewContainer(raccoon_VIEW_CONTAINER_ID)
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_CTRL_L_ACTION_ID,
			f1: true,
			title: localize2('raccoonCmdL', 'Raccoon AI: Add Selection to Chat'),
			keybinding: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// open panel
		const wasAlreadyOpen = viewsService.isViewContainerVisible(raccoon_VIEW_CONTAINER_ID)
		if (!wasAlreadyOpen) {
			await commandService.executeCommand(raccoon_OPEN_SIDEBAR_ACTION_ID)
		}
...
const raccoon_CMD_SHIFT_L_ACTION_ID = 'raccoon.cmdShiftL'
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: raccoon_CMD_SHIFT_L_ACTION_ID,
			title: 'New Chat',
			keybinding: {
			...
			icon: { id: 'add' },
			menu: [{ id: MenuId.ViewTitle, group: 'navigation', when: ContextKeyExpr.equals('view', raccoon_VIEW_ID), }],
		});
	}
	async run(accessor: ServicesAccessor): Promise<void> {
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		// Open a new thread which shows the landing page with history
		chatThreadsService.openNewThread();
		await chatThreadsService.focusCurrentChat();
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'raccoon.settingsAction',
			title: `Raccoon's Settings`,
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		const commandService = accessor.get(ICommandService)
		commandService.executeCommand(raccoon_TOGGLE_SETTINGS_ACTION_ID)
	}
})
