...
import { FeatureName, ModelSelection, ModelSelectionOptions } from '../common/raccoonSettingsTypes.js';
...
import { raccoonFileSnapshot } from '../common/editCodeServiceTypes.js';
...
export const IChatThreadService = createDecorator<IChatThreadService>('raccoonChatThreadService');
class ChatThreadService extends Disposable implements IChatThreadService {
	...
	private _getCheckpointInfo = (checkpointMessage: ChatMessage & { role: 'checkpoint' }, fsPath: string, opts: { includeUserModifiedChanges: boolean }) => {
		const raccoonFileSnapshot = checkpointMessage.raccoonFileSnapshotOfURI ? checkpointMessage.raccoonFileSnapshotOfURI[fsPath] ?? null : null
		if (!opts.includeUserModifiedChanges) { return { raccoonFileSnapshot, } }

		const userModifiedraccoonFileSnapshot = fsPath in checkpointMessage.userModifications.raccoonFileSnapshotOfURI ? checkpointMessage.userModifications.raccoonFileSnapshotOfURI[fsPath] ?? null : null
		return { raccoonFileSnapshot: userModifiedraccoonFileSnapshot ?? raccoonFileSnapshot, }
	}
	...
	private _computeNewCheckpointInfo({ threadId }: { threadId: string }) {
		...
		const raccoonFileSnapshotOfURI: { [fsPath: string]: raccoonFileSnapshot | undefined } = {}
		...
		for (const fsPath in lastIdxOfURI ?? {}) {
			const { model } = this._raccoonModelService.getModelFromFsPath(fsPath)
			if (!model) continue
			...
			if (!res) continue
			const { raccoonFileSnapshot: oldraccoonFileSnapshot } = res
			...
			const raccoonFileSnapshot = this._editCodeService.getraccoonFileSnapshot(URI.file(fsPath))
			if (oldraccoonFileSnapshot === raccoonFileSnapshot) continue
			raccoonFileSnapshotOfURI[fsPath] = raccoonFileSnapshot
		}
		...
		return { raccoonFileSnapshotOfURI }
	}


	private _addUserCheckpoint({ threadId }: { threadId: string }) {
		const { raccoonFileSnapshotOfURI } = this._computeNewCheckpointInfo({ threadId }) ?? {}
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'user_edit',
			raccoonFileSnapshotOfURI: raccoonFileSnapshotOfURI ?? {},
			userModifications: { raccoonFileSnapshotOfURI: {}, },
		})
	}
	...
	private _addToolEditCheckpoint({ threadId, uri, }: { threadId: string, uri: URI }) {
		...
		const diffAreasSnapshot = this._editCodeService.getraccoonFileSnapshot(uri)
		this._addCheckpoint(threadId, {
			role: 'checkpoint',
			type: 'tool_edit',
			raccoonFileSnapshotOfURI: { [uri.fsPath]: diffAreasSnapshot },
			userModifications: { raccoonFileSnapshotOfURI: {} },
		})
	}
	...
	private _getCheckpointsBetween({ threadId, loIdx, hiIdx }: { threadId: string, loIdx: number, hiIdx: number }) {
		...
		for (let i = loIdx; i <= hiIdx; i += 1) {
			...
			for (const fsPath in message.raccoonFileSnapshotOfURI) { // do not include userModified.beforeStrOfURI here, jumping should not include those changes
	...
	private _addUserModificationsToCurrCheckpoint({ threadId }: { threadId: string }) {
		...
		this._editMessageInThread(threadId, checkpointIdx, {
			...checkpoint,
			userModifications: { raccoonFileSnapshotOfURI: raccoonFileSnapshotOfURI ?? {}, },
		})
	}
		...
		if (toIdx < fromIdx) {
			...
			for (const fsPath in lastIdxOfURI) {
				...
				for (const k of idxes()) {
					...
					if (!res) continue
					const { raccoonFileSnapshot } = res
					if (!raccoonFileSnapshot) continue
					this._editCodeService.restoreraccoonFileSnapshot(URI.file(fsPath), raccoonFileSnapshot)
					break
				}
			}
		}
	...
	generateCodespanLink: IChatThreadService['generateCodespanLink'] = async ({ codespanStr: _codespanStr, threadId }) => {
		...
		if (codespanType === 'function-or-class') {
			...
			for (const uri of prevUris) {
				...
				const modelRef = await this._raccoonModelService.getModelSafe(uri)
