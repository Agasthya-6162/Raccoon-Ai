...
import { ChatMode, FeatureName, ModelSelection } from '../common/raccoonSettingsTypes.js';
import { ProviderName } from '../common/raccoonConstants.js';
...
const prepareOpenAIOrAnthropicMessages = ({
	...
	if (aiInstructions) sysMsgParts.push(`GUIDELINES (from the user's .raccoonrules file):\n${aiInstructions}`)
			...
			if (typeof m.content === 'string') {
				...
				const parts: GeminiModelPart[] = m.content.map((c): GeminiModelPart | null => {
					if (c.type === 'text') {
						...
						return { text: c.thinking, thought: true } as GeminiModelPart
					}
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	constructor(
		...
		@IRaccoonModelService private readonly raccoonModelService: IRaccoonModelService,
		@IMCPService private readonly mcpService: IMCPService,
	) {
		super()
	}

	// Read .raccoonrules files from workspace folders
	private _getraccoonRulesFileContents(): string {
		try {
			const workspaceFolders = this.workspaceContextService.getWorkspace().folders;
			let raccoonRules = '';
			for (const folder of workspaceFolders) {
				const uri = URI.joinPath(folder.uri, '.raccoonrules')
				const { model } = this.raccoonModelService.getModel(uri)
				if (!model) continue
				raccoonRules += model.getValue(EndOfLinePreference.LF) + '\n\n';
			}
			return raccoonRules.trim();
		}
		catch (e) {
			return ''
		}
	...
	// Get combined AI instructions from settings and .raccoonrules files
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.raccoonSettingsService.state.globalSettings.aiInstructions;
		const raccoonRulesFileContent = this._getraccoonRulesFileContents();

		const ans: string[] = []
		if (globalAIInstructions) ans.push(globalAIInstructions)
		if (raccoonRulesFileContent) ans.push(raccoonRulesFileContent)
		return ans.join('\n\n')
	}
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.raccoonSettingsService.state
		...
		const modelSelectionOptions = this.raccoonSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.raccoonSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.raccoonSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
