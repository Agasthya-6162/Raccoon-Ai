...
import { mountraccoonSelectionHelper } from './react/out/raccoon-editor-widgets-tsx/index.js';
...
export type raccoonSelectionHelperProps = {
...
export class SelectionHelperContribution extends Disposable implements IEditorContribution, IOverlayWidget {
	public static readonly ID = 'editor.contrib.raccoonSelectionHelper';
	// react
	...
	constructor(
		...
		@IRaccoonSettingsService private readonly _raccoonSettingsService: IRaccoonSettingsService
	) {
		super();
		...
		this._instantiationService.invokeFunction(accessor => {
			...
			const res = mountraccoonSelectionHelper(content, accessor);
			if (!res) return;
	...
	private _showHelperForSelection(selection: Selection): void {
		...
		// rerender
		const enabled = this._raccoonSettingsService.state.globalSettings.showInlineSuggestions
		...
		if (enabled) {
			this._rerender({ rerenderKey: this._rerenderKey } satisfies raccoonSelectionHelperProps)
