...
/* eslint-disable */ // raccoon AI
import { raccoon_CTRL_K_ACTION_ID, raccoon_CTRL_L_ACTION_ID } from '../../../contrib/raccoon/browser/actionIDs.js';
...
export class EditorGroupWatermark extends Disposable {
	...
	private render(): void {
		...
		this.clear();
		const raccoonIconBox = append(this.shortcuts, $('.watermark-box'));
		...
		const update = async () => {
			...
			if (this.contextService.getWorkbenchState() === WorkbenchState.EMPTY) {
				...
				raccoonIconBox.appendChild(buttonContainer);

				// Open a folder
				const openFolderButton = h('button')
				openFolderButton.root.classList.add('raccoon-openfolder-button')
				...
				openSSHButton.root.classList.add('raccoon-openssh-button')
				...
				if (recentlyOpened.length !== 0) {
					...
					raccoonIconBox.append(
						...recentlyOpened.map((w, i) => {
							...
							const linkSpan = $('span');
							linkSpan.classList.add('raccoon-link')
				...
				const keys = this.keybindingService.lookupKeybinding(raccoon_CTRL_L_ACTION_ID);
				const dl = append(raccoonIconBox, $('dl'));
				...
				const keys2 = this.keybindingService.lookupKeybinding(raccoon_CTRL_K_ACTION_ID);
				const dl2 = append(raccoonIconBox, $('dl'));
				...
				// 	label3.set(keys3);
				// button3.onclick = () => {
				// 	this.commandService.executeCommand(raccoon_OPEN_SETTINGS_ACTION_ID)
				// }
