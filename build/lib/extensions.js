...
function doPackageLocalExtensionsStream(forWeb, disableMangle, native) {
    ...
    if (forWeb) {
        ...
        const dependenciesSrc = productionDependencies.map(d => path_1.default.relative(root, d).replace(/\\/g, '/')).map(d => [`${d}/**`, `!${d}/**/{test,tests}/**`]).flat();
        result = event_stream_1.default.merge(localExtensionsStream, gulp_1.default.src(dependenciesSrc.length ? dependenciesSrc : ['.INVALID_HACK_TO_PREVENT_EMPTY_GLOB'], { base: '.', allowEmpty: true })
